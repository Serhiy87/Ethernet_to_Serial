...
#define HOST_NAME "WIZnet"
          uint32_t respId;
static uint8_t TD_DHCP_response_timer, TD_DHCP_lease_timer ;
enum
{
  padOption   = 0,
  subnetMask1    = 1,
...
uint8_t send_DHCP_MESSAGE_Automat(uint8_t event, uint8_t messageType, uint16_t secondsElapsed){
   ...
   uint8_t stateOld=state;
    #ifdef send_DHCP_MESSAGE_Automat_LOGGING
        SerialPrint("send_DHCP_MESSAGE_Automat state:") ;
...
void DHCP_init(){
	TD_DHCP_response_timer = Timer16SysAlloc(1);
	TD_DHCP_lease_timer = Timer16SysAlloc(1); 
	DHCP_automat(0);
};
...
//   uint16_t numBytes=0;
   uint8_t messageType = 0;
        ...
        SerialPrint("DHCP_automat state:") ;
        SerialPrintUint8_t(state) ;
		SerialPrintEndl();
    #endif
    switch(event){
      case 0:
	  if(TD_DHCP_response_timer == 0){
	  	TD_DHCP_response_timer = Timer32SysAlloc(1);
	  	TD_DHCP_lease_timer = Timer32SysAlloc(1);
		}
        state=0;
        break;
      default:
     ...
     switch(state)
     {
      ...
      case 1:
        ...
        if(UDPbegin0_Automat(1, DHCP_PORT)!=255)
        {
          break;
         }
         state=3;            
      ...
      case 3: 
        if(send_DHCP_MESSAGE_Automat(1, DHCP_DISCOVER, 10)!=255)
        {
             break;
        }
      ...
      case 5:
          ...
          if(messageType==DHCP_OFFER){
		   ...
		   if(Timer32Stopp(TD_DHCP_response_timer)){
		   		state=1;
      ...
      case 6:
           if(send_DHCP_MESSAGE_Automat(1, DHCP_REQUEST, 100)!=255)
            {
                 break;
            }
          	_dhcp_state=STATE_DHCP_REQUEST;
		 	StartTimer32(TD_DHCP_response_timer, 500);
           	state=7;
      		break;
    ...
    #ifdef DHCP_Automat_LOGGING
        SerialPrint("DHCP_automat_End_state:") ;
        SerialPrintUint8_t(state) ;
		SerialPrintEndl();
    #endif
      return state;
